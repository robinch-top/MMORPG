            ...
            if (!(player is null))
            {
                ...
                if (panel.activeSelf)
                {
                    ...
                    for (int i = 0; i < player.skills.skills.Count; i++)
                    {
                        ...
                        Skill.Skill skill = player.skills.skills[i];
                        ...
                        {
                            ((Player.PlayerSkills)player.skills).TryUse(icopy);
                        });
                        ...
                        // 学习或升级技能
                        if (skill.level < skill.maxLevel && ((Player.PlayerSkills)player.skills).CanUpgrade(skill))
                        {
                            ...
                            slot.upgradeButton.onClick.SetListener(() => { ((Player.PlayerSkills)player.skills).CmdUpgrade(icopy); });
                        }
                    ...
                    // 技能学习经验点
                    skillExperienceText.text = ((Player.PlayerSkills)player.skills).skillExperience.ToString();
                }
            }
