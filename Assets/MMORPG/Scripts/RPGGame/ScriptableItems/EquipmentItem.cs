...
namespace Assets.MMORPG.Scripts.RPGGame.ScriptableItems
{
    ...
    public class EquipmentItem : UsableItem
    {
        ...
        int FindEquipableSlotFor(Entity.Player player, int inventoryIndex)
        {
        ...
        public bool CanEquip(Entity.Player player, int inventoryIndex, int equipmentIndex)
        {
            Data.EquipSlotInfo slotInfo = player.equipment.slotInfo[equipmentIndex];
        ...
        public override void Use(Entity.Player player, int inventoryIndex)
        {
            ...
            if (equipmentIndex != -1)
            {
                Items.ItemSlot inventorySlot = player.inventory.slots[inventoryIndex];
                Items.ItemSlot equipmentSlot = player.equipment.slots[equipmentIndex];
                ...
                {
                    player.equipment.MergeInventoryEquip(inventoryIndex, equipmentIndex);
                }
                // 从背包到装备界面交换装备
                else
                {
                    player.equipment.SwapInventoryEquip(inventoryIndex, equipmentIndex);
                    //Debug.Log("换上装备");
                }
        ...
        public int FindEquipSlot(Entity.Player player)
        {
            for (int i = 0; i < player.equipment.slots.Count; ++i)
