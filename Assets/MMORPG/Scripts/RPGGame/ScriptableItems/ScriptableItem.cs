...
namespace Assets.MMORPG.Scripts.RPGGame.ScriptableItems
{
    ...
    public class ScriptableItem : Base.ScriptableNonAlloc
    {
        /// <summary>
        /// 道具最大堆叠数
        /// </summary>
        ...
        public static Dictionary<int, ScriptableItem> dict
        {
            get
            {
                ...
                if (cache == null)
                {
                    ...
                    // 检查重复项，然后添加到缓存
                    List<string> duplicates = Base.Extensions.FindDuplicates(items.ToList(), item => item.name);
                        //items.ToList().FindDuplicates(item => item.name);
